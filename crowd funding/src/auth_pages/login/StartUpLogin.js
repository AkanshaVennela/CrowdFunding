...
export default function StartUpLogin() {
  ...
  const [data, setData] = useState({ name: "start up",
    email: "", password: "", });
  ...
  const submitHandler = async (e) => {
    ...
    try { const res = await crowdFund.methods.startupUsersList(email).call();
      console.log(res);
      if (res.password === password) {
        localStorage.setItem("username", res.username);
        localStorage.setItem("email", email);
        navigate("/startup-home");
      } else { return alert("wrong user credintinals or please signup");}
  ...
  return ( <> <Navbar />
      <div className="registration_and_login"> <center>
